FindClick(ImageFile="", Options="", ByRef FoundX="", ByRef FoundY="") ; updated November 16, 2013 ... http://www.autohotkey.com/board/topic/89592-findclick-2nd-edition
{
	Static Cache
	, Center, Silent, Delim, Count, CharX, CharY, CharN, Sleep, Stay, Func, dx, d, m, x, y, o, n, k, r, e, a, w, t, f
	, ImageW, ImageH, LastX1, LastY1, ImageFilePath ; Does LastX/Y use the static ability? xxx
	...
	;----------------------------------------------------------------------------------------------------
	DefaultDirs = %A_AhkPath%\..\Graphics|%A_Desktop%|%A_ScriptDir%|%A_Temp%|C:\Dropbox\AutoHotkey\Graphics ; Pipe (|) delimited list of default directories to search if file not found in working directory
	...
	ForumURL = http://www.autohotkey.com/board/topic/89592-findclick-2nd-edition                            ; Url of forum
	DocumentationURL = https://www.cubby.com/pl/FindClick.pdf/_feba12f92a604faead036f157a95ba6b             ; Url of documentation file
	LastUpdate = November 16, 2013                                                                          ; Date of last update
	;||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
	If (ImageFile <> "") and (Asc(ImageFile) <> 62) {
		If (LastImageFile <> ImageFile) or (Options <> LastOptions) {
			...
			If (SpecialLaunch <> 3) {
				...
				, dx := "",               dx_user := True           ; Diagnostic Mode
				...
				;----------------------------------------------------------------------------------------------------
				; Change or add user configuration variables as you see fit.
			...
			If !(((ImageFile = @) and FileExist(ImageFilePath)) or %A_ThisFunc%(">ParseCache<", ImageFile)) {
				...
				If RegExMatch(ImageFile, "i)^\*(?:0x)?([0-9A-F]{2})([0-9A-F]{2})([0-9A-F]{2})(?:\D*(\d+)\D*(\d+).*?)?$", @) {
					...
					If !FileExist(ImageFilePath := A_Temp "\" A_ThisFunc "_" Color "_" ImageW "x" ImageH ".png") {
						...
						Loop % ImageW * ImageH
							Colors .= "|" Color
						%A_ThisFunc%(">MakeImage<", A_Temp "\" A_ThisFunc "_" Color "_" ImageW "x" ImageH "|png|" ImageW "|" ImageH Colors)
					}
				} Else {
					...
					If !%A_ThisFunc%(">ParseCache<", ImageFilePath) {
						If DxInfo
							DxInfo .= $ "2:Measuring width and height" #
							. "q:" DllCall("QueryPerformanceCounter", "Int64*", QPC) * QPC #
		...
		If (r <> "") {
			...
			If (Search1 = "") or (Search3 <= 0) or (Search4 <= 0) {
				...
				. "l:" A_LineNumber - 1 #
				. "d:2"
			Search3 += Search1, Search4 += Search2
		}
		If (a <> "") and (InStr(a, "m") <> 1) {
			...
			Loop, Parse, a, |`,
				...
				{
					Search%A_Index% := (A_LoopField >= 0 ? (A_Index & 1 ? Search1 : Search2) : (A_Index & 1 ? Search3 : Search4)) + A_LoopField
					If DxInfo
		...
		Loop {
			...
			If (SubRegions <> "") {
				...
				If DxInfo
					...
					. "c2:MouseX:" Search1 #
					. "c3:MouseY:" Search2 #
					. "g:Color,Red;Trans,200;Add,Picture,x1 y5 w30 h-1," A_WinDir "\Cursors\larrow.cur;Show,x" Search1 - 20 " y" Search2 - 20 " w40 h40 NA"
			...
			If (d = "") or (SubRegions <> "`n") {
				If DxInfo
					...
					. "q:" DllCall("QueryPerformanceCounter", "Int64*", QPC) * QPC #
					. "t:The following line of code will execute:`n`n%Code_Orig%`n`nType of region: " (Search5 ? Search5 : "Normal") #
					. "l:" A_LineNumber + 2 #
					...
					. "g:Color,FF9999,FF9999;Trans,80;Show,NA x" Search1 " y" Search2 " w" Search3 - Search1 + e1 + 1 " h" Search4 - Search2 + e2 + 1
			...
			If (ErrorLevel = 2) {
				...
				If (e <> "") {
					...
					If DxInfo
						...
						. "t:Because the ""e"" option was indicated, " A_ThisFunc "() needs to continue searching the screen for other instances of the image.`nThe following regions remain to be searched:`n`n<table>Top Left;Bottom Right;Region Type`n" SubRegions "</table>`n`n(Each time an image is found, the remaining space must be split into up to 3 additional regions.)" #
						. "l:" A_LineNumber - 7
					If d
						If (Asc(d) = 108) { ; l (Left)
					...
					Break
			} Else If (w = "") or SubRegions or (w1 and (A_TickCount > w1)) { ; XXX
				If DxInfo
					DxInfo .= $ "2:Search #" A_Index @ ; xxx 6/12/2013
					. $ "3:Image Not Found" #
					. "t:The image was not found in the given region.`n`n" (Results ? (SubRegions = "" ? "No more regions remain to be searched (""e"" option) so the script will now finish." : "Moving on to the next region:`n" SubStr(SubRegions, 1, InStr(SubRegions, "`n") - 1)) : w = "" ? "Because the ""w"" option (wait for an image to be found) was not used, the script will now finish." : w1 ? "The ""w"" option was indicated as " w " and " A_ThisFunc "() has continued to search for the image. However the wait time has now expired and the script will finish.") #
					. "g:Color,Red;Trans,80;Show,NA x" Search1 " y" Search2 " w" Search3 - Search1 + e1 + 1 " h" Search4 - Search2 + e2 + 1 #
					. "d:3" #
					. "l:" A_LineNumber - 2
				;~ If (SubRegions = "") ; removed to make directional ("d" option) work
					;~ Break
			} Else If Results {
				MsgBox, 262144, %A_ScriptName%: Alert, Problem! See line %a_linenumber% ; xxx
			} Else If w2 {
				If DxInfo
					DxInfo .= $ "2:Search #" A_Index @
					. "`r3:Sleeping" #
					. "q:" DllCall("QueryPerformanceCounter", "Int64*", QPC) * QPC #
					. "t:The ""w"" option (wait for an image to be found) was given as: %w%.`n`n<table>Total wait time;" SubStr(w, 1, InStr(w ",", ",") - 1) "`nSleep interval between searches;" (InStr(w, ",") ? SubStr(w, InStr(w, ",") + 1) "</table>" : "Not given</table>`n`nIf the sleep interval is not given then it will be assumed as either the total wait time divided by 10 or 100 milliseconds, whichever is smaller.") "`n`nSince no image has been found yet, the function now sleeps " w2 " milliseconds." #
					. "l:" A_LineNumber + 1 #
					. "c1:w:" w
				Sleep w2
		...
		If Results and n
			If (InStr(m, "c") = 1) {
				...
				If DxInfo
					...
					. "t:yyyy" #
					. "q:" DllCall("QueryPerformanceCounter", "Int64*", QPC) * QPC #
					. "g:Color,Red;Trans,200;Add,Picture,x1 y5 w30 h-1," A_WinDir "\Cursors\larrow.cur;Show,x" MouseX - 20 " y" MouseY - 20 " w40 h40 NA" #
					. "l:" A_LineNumber - 3
			}
		If DxInfo
	...
	Else If ImageFile = >MakeImage< ;--------------------------------------------------Builds an image file using GDI+--------------------------------------------------
	{
		...
		sOutput := Param1 "." Param2, VarSetCapacity(si, 16, 0), si := Chr(1), DllCall("gdiplus\GdiplusStartup", "uint*", pToken, "uint", &si, "uint", 0), DllCall("gdiplus\GdipCreateBitmapFromScan0", "int", Param3, "int", Param4, "int", 0, "int", 0x26200A, Ptr, 0, PtrA, pBitmap)
		Loop, Parse, Options, |
			DllCall("gdiplus\GdipBitmapSetPixel", Ptr, pBitmap, "int", Mod(A_Index - 1, Param3), "int", (A_Index - 1) // Param3, "int", "0xFF" A_LoopField)
	...
	Else If (InStr(ImageFile, ">Debug:") = 1) ;--------------------------------------------------Debugging--------------------------------------------------
	{
		...
		;----------------------------------------------------------------------------------------------------
		DebugSettingsLocation = %A_LineNumber%
		TextBoxFontSize = 9
		...
		;
		MenuFile = &%A_ThisFunc%
		MenuFilePrev = &Previous Step ; xxx was Pre&vious Step but I changed it... issues?
		...
		MenuSettingsAutoClose = Close When Finished ; xxx
		;
		MenuHelp = &Help
		...
		;
		EditorPath = %A_AhkPath%\..\SciTE\SciTE.exe
		EditorCommand = -goto:`%LineNumber`%
		;---------------------------------------------------- Default values ----------------------------------------------------
		CompactView = %False% ; xxx doesn't actually work if true
		ShowChars = %True%
		ShowGraphics = %True%
		AlwaysOnTop = %False%
		WrapText = %True% ; xxx
		...
		RecycleOnOverwrite = %True%              ; When overwriting an output file, the existing file will be recycled instead of deleted permanently
		...
		;||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
		CenterX := SquaresWide // 2, CenterY := SquaresHigh // 2, OffsetX := 25, OffsetY := 25, Cache := ">2" Cache, OptionInfo := SubStr(%A_ThisFunc%(A_Space), 2), WinDelay := A_WinDelay
		...
		Loop {
			...
			Loop {
				...
				If (TempGuiCommand = "ErrorLevel") {
						...
						While (GuiCommand = "~LButton") {
							...
							While (SquaresHigh >= @ := A_Index)
								Loop %SquaresWide% {
									...
									DllCall("gdiplus\GdipBitmapGetPixel", Ptr, pBitmap, "int", CenterX + A_Index - Floor(SquaresWide / 2) - 1, "int", CenterY + @ - Floor(SquaresHigh / 2) - 1, "uint*", Color), Color := Color & 0x00ffffff
					...
					;------------------------------------------------------- End GDI+ -------------------------------------------------------
					Gui, %GuiA%:Show, % "h" Margins * 16 + 1 + ControlHeight * 5 + (SquareSize + SquareSpacing) * SquaresHigh - SquareSpacing
